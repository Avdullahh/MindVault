...
export function EventItem({ event, onToggleDone, onEdit, onDelete }: Props) {
  return (
    <View className={`bg-gray-800 rounded-xl px-3 py-3 mb-2 flex-row items-start gap-3 border border-gray-700 min-h-16 ${event.done ? 'opacity-60' : ''}`}>
      <Pressable
      ...
      >
        <View className={`w-7 h-7 rounded-full border-2 items-center justify-center ${event.done ? 'bg-teal-500 border-teal-500' : 'border-gray-500'}`}>
      ...
      >
        <Text className={`font-medium ${event.done ? 'text-gray-500 line-through' : 'text-white'}`} numberOfLines={1}>
          {event.title}
        </Text>
        {event.notes ? <Text className="text-gray-400 text-sm mt-0.5" numberOfLines={1}>{event.notes}</Text> : null}
      </Pressable>
